...
private:
    void current_position_callback() {
        ...
        message_1.data =
                std::to_string(current_pos.pose.position.x) + "|" + std::to_string(current_pos.pose.position.y);
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (arg == "-id") {
            try {
                ...
                error_message =
                        error_message + "Order ID should be a number. Please use some number (int type) for order ID. ";
            }
        }
        if (arg == "-de") {
            try {
        ...
        if (arg == "-x") {
            try {
                ...
                error_message = error_message +
                                "X coordinate should be a number. Please use some number (float type) for x coordinate. ";
            }
        }
        if (arg == "-y") {
            try {
                ...
                error_message = error_message +
                                "Y coordinate should be a number. Please use some number (float type) for y coordinate.";
            }
        }
    }
    if (parameters != 4) {
        ...
        std::cerr
                << "Usage: ros2 run order_optimizer SendOrder -id <order id> -de <order description> -x <x coordinate> -y <y coordinate>"
                << std::endl;
        return 1;
    }
